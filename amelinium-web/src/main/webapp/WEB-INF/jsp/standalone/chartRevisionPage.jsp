...
<html>
<head>
<title><c:out value="${projectName}"/> Chart - Revision <c:out value="${chart.revision}"/> </title>
</head>
...
<td><a href="${path}/web/project/<c:out value="${projectId}"/>/chart/history/0">CHART HISTORY</a></td>
	...
	<a href="${path}/web/project/<c:out value="${projectId}"/>/chart/r<c:out value="${chart.revision}"/>/revert">restore this version</a>
</div>

<b><i><c:out value="${projectName}"/> Chart - Revision <c:out value="${chart.revision}"/></i></b></br>
	<pre><c:out value="${chart.content}" escapeXml="false" /></pre>
</body>
</html>