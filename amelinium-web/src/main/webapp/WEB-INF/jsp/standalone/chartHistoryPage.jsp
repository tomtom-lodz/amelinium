...
<html>
<head>
<title><c:out value="${projectName}"/> chart history</title>
</head>
<body>
...
<td><a href="${path}/web/project/<c:out value="${projectId}"/>/chart/history/0">CHART HISTORY</a></td>
	...
	<br/>
	<b><i><c:out value="${projectName}"/></i> chart history</b><br/>
<a href="${path}/web/project/<c:out value="${projectId}"/>/chart/history/<c:out value="${translationNewer}"/>">newer</a>
	&nbsp;
...
<c:forEach var="snapshot" items="${history}">
				...
				<tr>
					...
					<td style="color: grey">
					<a
						href="${path}/web/project/<c:out value="${projectId}"/>/chart/r<c:out value="${snapshot.revision}"/>/view">r. <c:out value="${snapshot.revision}"/></a>
						...
						<td>
							<a
						href="${path}/web/project/<c:out value="${projectId}"/>/chart/r<c:out value="${snapshot.revision}"/>/revert">restore this version</a>
...
<a href="${path}/web/project/<c:out value="${projectId}"/>/chart/history/<c:out value="${translationOlder}"/>">older</a>
<br/>
<br/>
</body>
</html>